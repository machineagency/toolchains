...
function getStartEndCenter(prevPt, pt, nextPt, radius) {
  ...
  const angle = Math.acos(
    prevNorm[0] * nextNorm[0] + prevNorm[1] * nextNorm[1]
  );
...
export function pathToCubics(cmds) {
  ...
  for (let i = 1; i < cmds.length; i++) {
    ...
    if (typeof cmd[0] !== "string") {
      ...
      const nextPt =
        i === cmds.length - 1 ? pt : getCmdPt(cmds[i + 1], pt, prevPt);
...
export function path(cmds) {
  ...
  for (let i = 0; i < cubics.length; i++) {
    ...
    const chopIt = (info) => {
      ...
      info.upperIndex =
        info.ptIndex + bezier(cubics[i + 1], resolution).length - 1;
  ...
  toFilletAndChamfer.forEach((fillet) => {
    ...
    if (newPts) {
      ...
      pts = [...pts.slice(0, startIndex), ...toAdd, ...pts.slice(endIndex)];
...
export function bezier(ps, resolution) {
