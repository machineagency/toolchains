...
`)}}const F={inports:{pixelArr:{type:"array",value:null}},outports:{gcode:{type:"string",value:null}},state:{fast:30,slow:5,radius:20,height:35},ui:{displayName:"Velocity",width:200,height:100}};function N(c,t,e){function i(){let r=e.radius,d=.2,m=e.height/d,l=[100,100],f=4*Math.PI*r,p=c.pixelArr.value,s=new E;s.start(),s.extrude_rel();let o=0;for(let n=0;n<m;n++){n==0?(s.fanOff(),s.move(l[0]+r,l[1],d),s.cmd("G1 F1500 E6.5"),s.setTemp(215),s.setFeedrate(s.speeds.wall_outer)):n==1?(s.setFan(85),s.setFeedrate(s.speeds.wall_outer)):n==2?s.setFan(170):n==3&&s.setFan(255);let u=0;for(let h=0;h<2*Math.PI;h+=Math.PI/f){if(n>5){let G=p[Math.floor(o)][Math.floor(u)]>125?e.fast:e.slow;s.setFeedrate(G),u+=.5,u>p[0].length-1&&(u=0)}let v=r*Math.cos(h)+l[0],x=r*Math.sin(h)+l[1];s.extrude_xy(v,x)}n>5&&(o+=.5,o>p.length-1&&(o=0)),s.zInc()}s.end();let g=s.gcode();t.gcode.value=g}function a(){return M`<style>
        .container {
          display: grid;
